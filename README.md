...
| [LiveCodeBench 2nd tier](reports/livecodebench-second-tier.md) | LiveCodeBench code-gen (28×1,055) | the **#1 is a real lead** (p=0.010) — but **ranks 2–6 are one statistical tie** (five models); top-10 = 3 tiers | paired t-test as a *positive control* — certifies a real gap, flags a fake one |
| [SWE-bench Verified top tie](reports/swebench-verified-top-tie.md) | SWE-bench Verified leaderboard | the benchmark separates mid-board, but **the top 8 submissions (79.2%→76.4%) are one statistical tie** with #1 (paired McNemar); #1 first separates at rank 9 | paired significance on a frontier coding leaderboard |
| [RewardBench category ties](reports/rewardbench-category-ties.md) | RewardBench per-category leaderboard | the aggregate ranking is sound, but **all four per-category #1s are statistically tied with #2** (paired McNemar) — the "#1 on Chat" gap is a coin flip | paired significance (McNemar) + a two-sided control that credits the headline |
...
**[#268](https://github.com/allenai/reward-bench/issues/268)** (v2 Focus),
**[#269](https://github.com/allenai/reward-bench/issues/269)** (per-category significance), and
